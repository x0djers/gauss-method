...
/**
  @brief Вычисляет произведение матрицы на число.
  ...
  NONE_ERROR, то поле matrix структуры MatrixOutcome будет равно NULL.
*/
MatrixOutcome multiplyNumberByMatrix(MATRIX_TYPE number, MatrixOutcome A);

MatrixErrorCode multiplyVectorByMatrix(const MATRIX_TYPE* inputVector,
									   size_t inputSize,
									   MatrixOutcome A,
									   MATRIX_TYPE** outputVector,
									   size_t* outputSize);
